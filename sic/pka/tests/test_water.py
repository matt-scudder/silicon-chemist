...
class WaterPkaTest(unittest.TestCase):
	...
	def testWaterPka(self):
		...
		for atom in water_mols[0]:
			if atom.atomicnum == 1:
				self.assertTrue(pka.get_pka(atom,water_mols[0]) == None)
			else:
				self.assertTrue(pka.get_pka(atom,water_mols[0]) == 15.7)
		for atom in water_mols[1]:
			if atom.atomicnum == 1:
				self.assertTrue(pka.get_pka(atom,water_mols[1]) == 15.7)
			else:
				self.assertTrue(pka.get_pka(atom,water_mols[1]) == -1.7)
		for atom in water_mols[2]:
			if atom.atomicnum == 1:
				self.assertTrue(pka.get_pka(atom,water_mols[2])== -1.7)
			else:
				self.assertTrue(pka.get_pka(atom,water_mols[2])== None)
def main():
	unittest.main()
