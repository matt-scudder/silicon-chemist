...
class LonePairIdentificationTest(unittest.TestCase):
    ...
    def testOxygenSources(self):
        ...
        for oxygen_pair in self.oxygen_pairs:
            ...
            self.assertTrue(len(results) == 1) #only one atom that's relevant
            self.assertTrue(len(results[0]) == 1) #only catch the lone pair atom

        for full_valence in self.full_oxygen_valence:
            mol = readstring("smi",full_valence)
            mol.addh()
            self.assertTrue(len(smarts.findall(mol)) == 0) #full valence means you shouldn't get anything
