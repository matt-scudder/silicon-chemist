...
class SN2(ReactionType):
    ...
    def cross_check(self):
        ...
        pka_BHNu = pka.get_pka(source["atoms"]["Y"]["atom"],source["atoms"]["Y"]["molecule"])
        ...
        new_mol = struct_ops.copy_molecule(source["atoms"]["Y"]["molecule"]) #the molecule passed in is arbitrary - remember it's the same for source and sink
        new_mol.break_bond(sink["atoms"]["C"],sink["atoms"]["L"]) #this works because we use indices, not direct atom.OBAtom refs
        new_mol.get_all_pka()
        pka_BHL = pka.get_pka(sink["atoms"]["L"],new_mol)
